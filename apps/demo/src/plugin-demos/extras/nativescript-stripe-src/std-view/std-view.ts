...
class Listener implements StripeStandardPaymentListener {
	...
	onPaymentDataChanged(data: StripeStandardPaymentData) {
		...
		if (data.shippingAddress) {
			...
			model.debugInfo += [`${data.shippingInfo.label} to:`, addr.name, addr.line1, addr.line2, `${addr.city}, ${addr.state} ${addr.country} ${addr.postalCode}`, addr.phone, addr.email].join('\n');
		}
	}
	...
	onPaymentSuccess(): void {
		...
		model.canBuy = true;
		model.successMessage = `Congratulations! You bought a "${model.item.name}" for $${model.item.price / 100}.`;
	}
	...
	provideShippingMethods(address: StripeStandardAddress): StripeStandardShippingMethods {
		const upsGround: StripeStandardShippingMethod = {
			currency: 'USD',
			amount: 0,
			label: 'UPS Ground',
		...
		const upsWorldwide: StripeStandardShippingMethod = {
			currency: 'USD',
			amount: 1099,
		...
		const fedEx: StripeStandardShippingMethod = {
			currency: 'USD',
			amount: 599,
			label: 'FedEx',
		...
		const methods = <StripeStandardShippingMethods>{};
