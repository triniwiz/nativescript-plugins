...
export const description: string = 'Basic building block of all UI, examples that ' + 'demonstrate some of the many styles available.';
		...
		render() {
			return (
				<View style={{ borderWidth: 0.5, borderRadius: 5, padding: 5 }}>
					<label style={{ fontSize: 11 }}>Too much use of `borderRadius` (especially large radii) on anything which is scrolling may result in dropped frames. Use sparingly.</label>
				</View>
			);
		},
	},
					...
					return (
						...
						<View onTap={this._handlePress}>
							<View>
								...
								<label style={{ paddingBottom: 10 }}>Same blobs, but their shared container have 0.5 opacity</label>
								<label style={{ paddingBottom: 10 }}>Tap to {this.state.active ? 'activate' : 'deactivate'} needsOffscreenAlphaCompositing</label>
								<View
									style={{ opacity: 0.8 }}
									/* Doesn't exist in NativeScript */
									// needsOffscreenAlphaCompositing={this.state.active}
								>
									<View style={{ marginTop: 0, marginLeft: 0, backgroundColor: '#FF6F59', ...styles.alphaCompositing }} />
									<View
										style={{
											marginTop: -50,
											marginLeft: 50,
			...
			class ZIndexExample extends React.Component<Props, State> {
				...
				render() {
					...
					return (
						...
						<View onTap={this._handlePress}>
							<View>
								<label style={{ paddingBottom: 10 }}>Tap to flip sorting order</label>
								<View
									style={{
										marginTop: 0,
