...
export default class RealtimeClient {
	...
	constructor(endPoint: string, options?: Options) {
		...
		if (options?.longpollerTimeout) this.longpollerTimeout = options.longpollerTimeout;
	...
	connect() {
		...
		if (this.conn) {
			...
			this.conn.onerror = (error) => this._onConnError(error as any);
	...
	remove(channel: RealtimeSubscription) {
		this.channels = this.channels.filter((c: RealtimeSubscription) => c.joinRef() !== channel.joinRef());
	}
	...
	onConnMessage(rawMessage: any) {
		this.decode(rawMessage.data, (msg: Message) => {
			...
			this.channels.filter((channel: RealtimeSubscription) => channel.isMember(topic)).forEach((channel: RealtimeSubscription) => channel.trigger(event, payload, ref));
	...
	private _triggerChanError() {
		this.channels.forEach((channel: RealtimeSubscription) => channel.trigger(CHANNEL_EVENTS.error));
	}
