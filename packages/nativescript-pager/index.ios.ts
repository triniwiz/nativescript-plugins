import { ChangeType, Color, EventData, ObservableArray, profile, ProxyViewContainer, StackLayout, Utils, View } from '@nativescript/core';
...
export class Pager extends PagerBase {
	...
	_layout: UICollectionViewFlowLinearLayoutImpl; /*UICollectionViewFlowLinearLayoutImpl*/
	...
	createNativeView() {
		...
		this._layout = UICollectionViewFlowLinearLayoutImpl.alloc().init();
		...
		this.pager.translatesAutoresizingMaskIntoConstraints = false;
	...
	_getRealWidthHeight(): { width: number; height: number } {
		let height = 0;
		let width = 0;
		const space = this.perPage * 2 * this._getSpacing() + this._getPeaking() * 2;
		width = (layout.toDeviceIndependentPixels(this._effectiveItemWidth) - space) / this.perPage;
		height = (layout.toDeviceIndependentPixels(this._effectiveItemHeight) - space) / this.perPage;
		return { height, width };
	}
	...
	private _updateSpacing() {
		if (this._layout) {
			this._layout.minimumLineSpacing = this._getSpacing();
		}
	}

	[common.spacingProperty.setNative](value) {
		this._updateSpacing();
	}

	private _updatePeaking() {
		if (this._layout) {
			let sectionInset;
			const inset = this._getPeaking();
			if (this.orientation === 'vertical') {
			...
			this._layout.sectionInset = sectionInset;
		}
	}

	[common.peakingProperty.setNative](value) {
		this._updatePeaking();
	}
	...
	[itemsProperty.setNative](value: any) {
		...
		if (value?.length !== this._lastItems?.length) {
	...
	_scrollToIndexAnimated(index: number, animate: boolean) {
		...
		this._isAnimatedScrolling = animate;
		this._scrollingToIndex = maxMinIndex;
	...
	public onLayout(left: number, top: number, right: number, bottom: number) {
		...
		this._layout.itemSize = CGSizeMake(size.width, size.height);
		this._updatePeaking();
		this._updateSpacing();
	...
	public _prepareCell(cell: PagerCell, indexPath: NSIndexPath) {
		try {
			...
			view.iosOverflowSafeArea = this.iosOverflowSafeArea;
			view.iosIgnoreSafeArea = this.iosIgnoreSafeArea;
	...
	_getSize(w: number = 0, h: number = 0): { width: number; height: number } {
		...
		if (this.orientation === 'vertical') {
			height = (height - this._getPeaking()) / this.perPage;
		} else {
			width = (width - this._getPeaking()) / this.perPage;
		}
	...
	[common.transformerProperty.setNative](value) {
		if (typeof value === 'string' && this._layout) {
			this._layout.transformer = value;
		}
	}
}
