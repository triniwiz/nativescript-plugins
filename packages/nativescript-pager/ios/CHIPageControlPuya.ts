...
@NativeClass
class CHIPageControlPuya extends UIControl implements CHIPageControllable {
...
Base Controller
  ...
  layoutSubviews() {
    ...
    if (active) {
      active.backgroundColor = (this.currentPageTintColor ?? this.tintColor).CGColor
    ...
    this.frames = this.elements.map(item => item.frame);
    this.update(this.progress)
  }
  ...
  update(progress: number) {
    ...
    if (indices.indexOf(page) && indices.indexOf(page + 1)) {
      return
    }

    let prev = this.frames[page]
    let prevColor = this.getTintColor(page)
    let current = this.frames[page + 1]
    let currentColor = this.getTintColor(page + 1);
