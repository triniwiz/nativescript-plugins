...
@NativeClass
class CHIPageControlAleppo extends UIControl implements CHIPageControllable {
  /*
    Base Controller
   */
...
Base Controller
  ...
  _initProps() {
    ...
    this._animationFrameImpl = AnimationFrameImpl.initWithOwner(new WeakRef<CHIPageControlAleppo>(this));
  ...
  layoutSubviews() {
    ...
    const x = (this.bounds.size.width - this.diameter * floatCount - this.padding * (floatCount - 1)) * 0.5
    const y = (this.bounds.size.height - this.diameter) * 0.5
    const frame = CGRectMake(x, y, this.diameter, this.diameter)
