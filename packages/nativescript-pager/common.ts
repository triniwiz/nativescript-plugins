...
export abstract class PagerBase extends ContainerView implements AddChildFromBuilder {
	...
	_getDataItem(index: number): any {
		...
		if (thisItems) {
			return thisItems && (<ItemsSource>thisItems).getItem ? (<ItemsSource>thisItems).getItem(index) : thisItems[index];
		}
	}
...
export const itemTemplatesProperty = new Property<PagerBase, string | Array<KeyedTemplate>>({
	name: 'itemTemplates',
	affectsLayout: true,
...
export const transformerProperty = new Property<PagerBase, string>({
	name: 'transformer',
});

transformerProperty.register(PagerBase);
