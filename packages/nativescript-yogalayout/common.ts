import { AddChildFromBuilder, Property, View as NSView, PercentLength, Style, CssProperty, PropertyChangeData } from '@nativescript/core';
...
export abstract class ViewBase extends NSView implements AddChildFromBuilder {
	...
	android: any;
	yoga: any;
	yogaNode: com.facebook.yoga.YogaNode;
	...
	_updateAspectRatio(value: number, force = false, json = null) {
		...
		if (this.nativeView) {
			if (global.isAndroid) {
				...
				this.nativeView.yoga.aspectRatio = value;
				if (force) {
					this.nativeView.yoga.applyLayoutPreservingOrigin(true);
				}
			}
	...
	_setStartOrEnd(value: number, position: 'start' | 'end', force = false, json = null) {
		if (this.nativeView) {
			let direction;
			if (global.isAndroid) {
				direction = io.github.triniwiz.yogalayout.Utils.getLayoutDirection(this.yogaNode);
			} else if (global.isIOS) {
				direction = this.nativeView.direction;
			}

			switch (direction) {
				case 'rtl':
					if (position === 'start') {
						this._updateRight(value, force, json);
					}
					if (position === 'end') {
						this._updateLeft(value, force, json);
					}
					break;
				case 'ltr':
					if (position === 'start') {
	...
	_updateStart(value: number, force = false, json = null) {
		this._setStartOrEnd(value, 'start', force, json);
	}

	_updateEnd(value: number, force = false, json = null) {
		this._setStartOrEnd(value, 'end', force, json);
	}

	_updateDirection(value: Direction, force = false, json = null) {
		if (!value) {
			return;
		}
		if (this.nativeView) {
			if (global.isAndroid) {
				...
				this.nativeView.yoga.direction = _toYGDirection(value);
			}
		}
	}

	_updateMarginVertical(value: any, force = false, json = null) {
		this._setMarginValue(value, 'vertical', json);
	}

	_updateMarginHorizontal(value: any, force = false, json = null) {
		this._setMarginValue(value, 'horizontal', json);
	}

	_updatePaddingHorizontal(value: any, force = false, json = null) {
		this._setPaddingValue(value, 'horizontal', json);
	}

	_updatePaddingVertical(value: any, force = false, json = null) {
		this._setPaddingValue(value, 'vertical', json);
	}
}
