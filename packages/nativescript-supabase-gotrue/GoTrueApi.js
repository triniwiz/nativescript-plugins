...
export default class GoTrueApi {
    ...
    async signInWithEmail(email, password, options = {}) {
        try {
            ...
            const data = await post(`${this.url}/token?grant_type=password`, { email, password }, { headers });
            let session = { ...data };
            if (session.expires_in)
                session.expires_at = expiresAt(data.expires_in);
            return { data: session, error: null };
    ...
    async signOut(jwt) {
        try {
            await post(`${this.url}/logout`, {}, { headers: this._createRequestHeaders(jwt), noResolveJson: true });
            return { error: null };
        }
        catch (error) {
            return { error };
    ...
    async getUser(jwt) {
        try {
            const data = await get(`${this.url}/user`, { headers: this._createRequestHeaders(jwt) });
    ...
    async updateUser(jwt, attributes) {
        try {
            const data = await put(`${this.url}/user`, attributes, {
     ...
     */
    async deleteUser(uid, jwt) {
        try {
            const data = await remove(`${this.url}/admin/users/${uid}`, {}, {
                headers: this._createRequestHeaders(jwt),
            });
            return { user: data, data, error: null };
        }
    ...
    async refreshAccessToken(refreshToken) {
        try {
            const data = await post(`${this.url}/token?grant_type=refresh_token`, { refresh_token: refreshToken }, { headers: this.headers });
            let session = { ...data };
            if (session.expires_in)
                session.expires_at = expiresAt(data.expires_in);
            return { data: session, error: null };
