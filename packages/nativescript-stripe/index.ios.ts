...
export class Token implements IToken {
	...
	private constructor(token: STPToken) {
		...
		this.card = token.card ? Card.fromNative(token.card) : null;
...
export class Stripe {
	...
	createBankAccountToken(account: BankAccountParams, cb: (error: Error, token: IToken) => void): void {
		if (!account) {
			if (typeof cb === 'function') {
				cb(new Error('Invalid bank account'), null);
			}
			return;
		}
		const apiClient = STPAPIClient.sharedClient;
		apiClient.createTokenWithBankAccountCompletion(
			account.native,
			callback(cb, (token: STPToken) => Token.fromNative(token))
		);
	}
...
export class BankAccountParams {
	private _bankAccountParams: STPBankAccountParams;

	constructor(...args) {
		if (args[0] instanceof STPBankAccountParams) {
			this._bankAccountParams = args[0];
		} else if (args.length === 5) {
			this._bankAccountParams = STPBankAccountParams.alloc().init();
			this._bankAccountParams.accountNumber = args[0];
			this._bankAccountParams.accountHolderName = args[1];
			this._bankAccountParams.country = args[2];
			this._bankAccountParams.currency = args[3];
			this._bankAccountParams.accountHolderType = BankAccountHolderType.Company === args[4] ? STPBankAccountHolderType.Company : STPBankAccountHolderType.Individual;
		} else {
			this._bankAccountParams = STPBankAccountParams.alloc().init();
		}
	}
	static fromNative(account) {
		return new BankAccountParams(account);
	}
	get native() {
		return this._bankAccountParams;
	}
}
