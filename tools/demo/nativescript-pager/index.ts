...
export class DemoSharedNativescriptPager extends DemoSharedBase {
	...
	get toSlice() {
		const now = Date.now();
		return [
			{ title: `page-0`, image: '~/images/Hulk_(comics_character).png', items: this._items },
			{
				title: `page-1`,
				image: `https://robohash.org/${now}.png`,
				items: this._items,
			},
		];
	}
	...
	generateMore(count: number = 0) {
		...
		for (let i = 0; i < count; i++) {
			array.push({
				title: 'page-' + (this.items.length + (i + 1)),
	...
	getRandom(event) {
		const itemIndex = this.getRandomInt(0, this.items.length - 1);
    const item = this.pager.getViewById(`page-${itemIndex}`);
