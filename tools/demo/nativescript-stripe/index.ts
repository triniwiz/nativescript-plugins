...
import { CreditCardView, PaymentMethod, Source, Stripe, StripeSetupIntent, StripeThreeDSUICustomization, Token } from '@triniwiz/nativescript-stripe';
import { EventData, fromObject, Button, Page, Frame } from '@nativescript/core';
import { publishableKey, StripeService } from '../../../apps/demo/src/plugin-demos/extras/nativescript-stripe-src/std-view/stripe.service';
...
export class DemoSharedNativescriptStripe extends DemoSharedBase {
	...
	stdDemo(args: EventData) {
		Frame.topmost().navigate('~/plugin-demos/extras/nativescript-stripe-src/std-view/std-view');
	}

	ccViewDemo(args: EventData) {
		Frame.topmost().navigate('~/plugin-demos/extras/nativescript-stripe-src/cc-view/cc-view');
	}

	paymentSheetDemo(args) {
		Frame.topmost().navigate('~/plugin-demos/extras/nativescript-stripe-src/payment-sheet-view/payment-sheet-view');
	}
